...
class McpServerIntegrationTest {
    ...
    void promptCompletion_returnsMatchingNames() {
        // Prefix "V" should match "Varsha" and exclude "Harsha", proving the
        // completion filter is applied (candidates live in Prompt#completeConfig).
        McpSchema.CompleteResult result = client.completeCompletion(new McpSchema.CompleteRequest(
                new McpSchema.PromptReference("greeting"),
                new McpSchema.CompleteRequest.CompleteArgument("name", "V")));

        assertThat(result.completion().values()).containsExactlyInAnyOrder("Varsha");
    }
    ...
    void diceAppResource_servesHtmlWithMcpAppProfile() {
        ...
        // The MCP Apps bundle is inlined and its `App` export rewritten to a
        // top-level const (CSP-safe). This proves the bundle was injected AND
        // transformed, and that the CSP-blocked blob-import is gone.
        assertThat(contents.text()).contains("const App =");
        assertThat(contents.text()).doesNotContain("createObjectURL");
    }
    ...
    void shoppingListResource_servesHtmlWithMcpAppProfile() {
        ...
        // Bundle inlined + `App` export rewritten to a top-level const (CSP-safe),
        // and the CSP-blocked blob-import removed.
        assertThat(contents.text()).contains("const App =");
        assertThat(contents.text()).doesNotContain("createObjectURL");
    }
}
